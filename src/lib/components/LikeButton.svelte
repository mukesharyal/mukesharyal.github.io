<script>
    ...
    // This state variable becomes true when the captcha is loaded, and only then is the LikeButton actually shown
    ...
    {
        // As the captcha has loaded, now we show the LikeButton
    ...
    onMount(() => {
        ...
        return(() => {
            window.removeEventListener('turnstile-script-loaded', initializeTurnstile);
        });
    });
